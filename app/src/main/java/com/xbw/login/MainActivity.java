...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initView();
		SMSSDK.initSDK(this, mob_APPKEY, mob_APPSECRET);//mob的key
	...
	private void initView() {
		...
		mSignupBtn.setBackground(MainActivity.this.getResources().getDrawable(
				R.color.sign_up_btn_normal));
		mSendmsgBtn.setText("发送验证码");
		mSendmsgBtn.setClickable(false);
		mSendmsgBtn.setBackground(this.getResources().getDrawable(
				...
				if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) {
					if (result == SMSSDK.RESULT_COMPLETE) {
						Toast.makeText(getApplicationContext(), "短信验证成功",
								Toast.LENGTH_SHORT).show();
						mSignupBtn.setBackground(MainActivity.this
					...
					TimeCountUtil timeCountUtil = new TimeCountUtil(
							MainActivity.this, 60000, 1000, mSendmsgBtn);
					timeCountUtil.start();
	...
	TextWatcher myWatchers = new TextWatcher() {
		...
		public void afterTextChanged(Editable s) {
			if (mUsernameET.getText().length() == 11) {
				...
				success = false;
				mSignupBtn.setClickable(false);
				mSignupBtn.setBackground(MainActivity.this.getResources()
